...
def process_input(qin, subproc):
	...
	while True:
		if not qin.empty():
			input = qin.get()  # input() is a Python builtin thus a poor variable name
			src = input[0]
			dst = input[1]
			stream = input[2]
...
def process_bash_error(qout, subproc):
	...
	while True:
		...
		if errstream in r:
			...
			if byte == "\n":
				# full line received from subprocess, send it to HID
				#   note: the newline char isn't send, as each outputstream is printed in a
				#         separate line by the powershell client
